...
def parse_composer( efd, item ):
    ...
    composer = parse( efd, 'composer', item, r'<strong>Composer\(s\):</strong> <a.*?>(.*?)</a>' )
    ...
    if found_slash:
        extra_composer = parse( efd, 'extra_composer', item, r'<strong>Composer\(s\):</strong> <a.*?>.*?</a>.*?<a.*?>(.*?)</a>' )
...
def proc_item( fb, efd, item ):
    ...
    lyricist = parse( efd, 'lyricist', item, r'<strong>Lyricist:</strong>\s*(.*?)\n' )
    book = parse( efd, 'book', item, r'<strong>Fakebook:</strong> <a.*?>(.*?)</a><br>' )
    sheet = parse( efd, 'page', item, r'<strong>Page:</strong>\s*(.*?)<br>' )
