# birdland-qt.onedir.spec
# ----------------------------------------------------------------------------------------
#   birdland-qt.spec - WRW 26-Mar-2025  - finally trying to package birdland-qt
...
# ----------------------------------------------------------------------------------------
#   WRW 22-May-2025 - trying to include vlc in bundle.
#   This is probably way overkill but at least try it.
#   [('dest_path', 'source_path', 'BINARY')]

def collect_vlc_files():
    res = []

    root = Path("/usr/lib")
    for path in root.rglob("libvlc*"):
        if path.is_file():
            part = path.relative_to( root )
            item = ( str(part), str(path), 'BINARY' )
            res.append( item )

    root = Path("/usr/lib/vlc")
    for path in root.rglob("*"):
        if str(path).endswith("plugins.dat"):
            continue

        if path.is_file():
            part = Path( 'vlc', path.relative_to( root ))
            item = ( str(part), str(path), 'BINARY' )
            res.append( item )
    return res
...
source_files = [
    os.path.join( src_dir, "birdstart.py"),             # Main entry point, dispatches to many files
...
#    ( 'lib/libvlc.so',     '/usr/lib/libvlc.so.5.6.1',     'BINARY' ),
#    ( 'lib/libvlccore.so', '/usr/lib/libvlccore.so.9.0.1', 'BINARY' ),
#    ( 'libvlc.so',         '/usr/lib/libvlc.so.5.6.1',     'BINARY' ),
#    ( 'libvlccore.so',     '/usr/lib/libvlccore.so.9.0.1', 'BINARY' ),
]

#   From pyi.linux.onedir.spec, not sure if something analogous need for windows.
#   vlc = collect_vlc_files()
#   a.binaries += vlc
...
exe = EXE(
    ...
    icon='src/Icons/Saxophone_64.ico',       # WRW 27-Mar-2025 - Added, must be .ico
)
